...
dates as (
SELECT ExplodedDate = DATEADD(DAY,N - 1,'2000-01-01')
FROM cteTally
WHERE N <= 18300
)
...
BEGIN
	...
	WHEN NOT MATCHED BY TARGET
	THEN 
	INSERT (CustomerName,CustomerAddress,CustomerPhone, IsActual, CreateDate, ChangeDate, DeleteDate)
	VALUES (CustomerName,CustomerAddress,CustomerPhone, 1, SYSDATETIME(),SYSDATETIME() ,'2099-01-01' );
	...
	USING
		(
			SELECT
				...
				FROM dbo.part P
				INNER JOIN dbo.partsupp PS on P.p_partkey = PS.ps_partkey
		) AS SOURCE
...
BEGIN
	...
	USING ( 
		...
		FROM dbo.orders O
			INNER JOIN dbo.lineitem L on O.o_orderkey = L.l_orderkey
			INNER JOIN dim.date DD on O.o_orderdate = DD.FullDate
			INNER JOIN dbo.customer C on O.o_custkey = C.c_custkey
			INNER JOIN dim.parts P on L.l_partkey = P.PartID
			INNER JOIN dbo.supplier S on L.l_suppkey = S.s_suppkey
			INNER JOIN dbo.nation NC on C.c_nationkey = NC.n_nationkey
			INNER JOIN dbo.region RC on NC.n_regionkey = RC.r_regionkey
			INNER JOIN dbo.nation NS on S.s_nationkey = NS.n_nationkey
			INNER JOIN dbo.region RS on NS.n_nationkey = RS.r_regionkey
		) as SOURCE
